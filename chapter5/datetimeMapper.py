...
def mapper():
    ...
    for line in sys.stdin:
        # Generate record from line
        data = line.strip().split(",")
        
        # Determine indices for the projection
        if isHeader:
            unitIx    = data.index("UNIT")
            entriesIx = data.index("ENTRIESn_hourly")
            dateIx    = data.index("DATEn")
            timeIx    = data.index("TIMEn")
            isHeader  = False
        else:
            unit    = data[unitIx]
            entries = data[entriesIx]
            date    = data[dateIx]
            time    = data[timeIx]
            print('\t'.join([unit,entries,date,time]))
