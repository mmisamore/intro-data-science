...
def mapper():
    ...
    for line in sys.stdin:
        ...
        # If header, lookup indices
        if isHeader:
            fogIx = data.index('fog')
            rainIx = data.index('rain')
            entries = data.index('ENTRIESn_hourly')
            isHeader = False
        else:
            # Generate k-v pair for each record
            fog = int(float(data[fogIx]))
            rain = int(float(data[rainIx]))
            k = format_key(fog,rain)
            print(k + '\t' + data[entries])
